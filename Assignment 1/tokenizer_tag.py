...
TOKEN_REGEX = re.compile(
    r'(?P<URL>' + URL_PATTERN + r')'
    r'|(?P<EMAIL>' + EMAIL_PATTERN + r')'
    r'|(?P<DATE>' + DATE_PATTERN + r')'
    r'|(?P<NUMBER>' + NUMBER_PATTERN + r')'
    r'|(?P<WORD>' + LATIN_WORD_PATTERN + r')'
    r'|(?P<WORD_DEV>' + DEVANAGARI_WORD_PATTERN + r')'
    r'|(?P<WORD_GUJ>' + GUJARATI_WORD_PATTERN + r')'
    r'|(?P<PUNCT>' + PUNCT_PATTERN + r')'
)

# Collapse the three word-ish group names into one label for callers that
# just want "is this a word or not" rather than which script it came from.
_GROUP_TO_TYPE = {
    "URL": "URL",
    "EMAIL": "EMAIL",
    "DATE": "DATE",
...
def word_tokenize_with_types(text: str):
    """Tokenize into (token, type) pairs. type is one of:
    URL, EMAIL, DATE, NUMBER, WORD, PUNCT."""
    return [
        (m.group(), _GROUP_TO_TYPE[m.lastgroup])
...
def word_tokenize(text: str):
    ...
    (no type info -- use word_tokenize_with_types if you need that)."""
    return [tok for tok, _ in word_tokenize_with_types(text)]
...
def tokenize_paragraph_with_types(paragraph: str):
    """Same as tokenize_paragraph but each token comes with its type:
    paragraph -> list[list[(token, type)]]."""
    sentences = sentence_tokenize(paragraph)
    return [word_tokenize_with_types(s) for s in sentences]
...
if __name__ == "__main__":
    ...
    for i, sent in enumerate(tokenize_paragraph_with_types(sample), 1):
        print(f"S{i}:", sent)